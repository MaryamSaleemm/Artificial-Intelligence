...
def Astar():
    ...
    while len(frontier) != 0:
        ...
        for child in graph[currentNode].actions:
            ...
            heuristicCost = math.sqrt(
                ((graph[goalState].heuristic[0] - graph[childState].heuristic[0]) ** 2) +
                ((graph[goalState].heuristic[1] - graph[childState].heuristic[1]) ** 2)
            )
            totalCost = newCost + heuristicCost

            if childState in explored and explored[childState][1] <= totalCost:
                continue

            if childState not in frontier or frontier[childState][1] > totalCost:
                graph[childState].parent = currentNode
                graph[childState].totalCost = newCost
